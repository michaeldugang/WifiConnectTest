...
public class WifiUtils {
	...
	private WifiConfiguration getWifiConfiguration(WifiManager wifiManager, String ssid) {
		...
		List<WifiConfiguration> configedNetworks = mWifiManager.getConfiguredNetworks();
		if (configedNetworks != null) {
			for (WifiConfiguration wifiConfiguration : configedNetworks) {
	...
	public boolean isNetworkConnected() {
		ConnectivityManager connMan = mContext.getSystemService(ConnectivityManager.class);
		...
		if (netInfo.isConnected() && netInfo.getType() == ConnectivityManager.TYPE_WIFI) {
			...
			Log.d(TAG, "currentConnection: " + currentConnection);
			if (DEBUG) {
	...
	public boolean connectWifiTest(final String ssid, final String pwd) {
		...
		Log.d(TAG, "connectWifiTest: ssid : " + ssid);
		mWifiConfiguration = getWifiConfiguration(mWifiManager, ssid);

		Log.d(TAG, "*********mWifiConfiguration : " + mWifiConfiguration);
		...
		boolean addSucess = addNetwork(mWifiConfiguration);
		if (addSucess) {
			if (!flag && !isSuccess) {/*
				try {
					Thread.sleep(10000);
